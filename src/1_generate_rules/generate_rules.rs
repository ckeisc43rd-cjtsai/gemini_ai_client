use google_generative_ai_rs::v1::gemini::request::{
    GenerationConfig, SystemInstructionContent, SystemInstructionPart,
};
...
async fn generate_rules(
    ...
    output_path: String,
    model_name: String,
    custom_generation_config: Option<GenerationConfig>,
    ...
    #[cfg(feature = "beta")]
    {
        ...
        let client: google_generative_ai_rs::v1::api::Client =
            Client::new_from_model(Model::Gemini1_5Pro, env::var("GEMINI_API_KEY").unwrap());
        ...
        let prompt = format!(
            "form_question: {}\nform_respond: {}",
            form_question_contents, form_respond_contents
        );
        log::info!("Prompt: {:#?}", prompt);

        if (custom_generation_config == None) {
            custom_generation_config = Some(GenerationConfig {
        ...
        println!(
            "response: {}",
            response.rest().unwrap().candidates[0].content.parts[0]
                .text
                .as_ref()
                .unwrap()
        );
